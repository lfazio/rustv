...
impl PeripheralInterface for Sram {
...
mod tests {
    ...
    #[test]
    fn getters() {
        let sram: Sram = Sram::new(16);

        assert_eq!(sram.size(), 16);
    }
    ...
    fn fetch_above_length() {
        ...
        assert!(sram.fetch(1, 0x10).is_err());
        assert!(sram.fetch(2, 0x10).is_err());
        assert!(sram.fetch(4, 0x10).is_err());
        assert!(sram.fetch(8, 0x10).is_err());

        assert!(sram.fetch(1, 0x10).is_err());
        assert!(sram.fetch(2, 0x0f).is_err());
        assert!(sram.fetch(4, 0x0d).is_err());
        assert!(sram.fetch(8, 0x09).is_err());
    }
    ...
    fn store_fetch_width4() {
        ...
        assert!(sram.fetch(4, 0x1).is_err());

        /* Unaligned access */
        match sram.store(4, 0x2, &u32::to_le_bytes(0x12345678).to_vec()) {
            Some(BusException::StoreAddressMisaligned) => (),
            _ => assert!(false),
        }
        assert!(sram.fetch(4, 0x2).is_err());

        /* Unaligned access */
        match sram.store(4, 0x3, &u32::to_le_bytes(0x12345678).to_vec()) {
            Some(BusException::StoreAddressMisaligned) => (),
            _ => assert!(false),
        }
    ...
    fn store_fetch_width8() {
        ...
        assert!(sram.fetch(8, 0x1).is_err());

        /* Unaligned access */
        match sram.store(8, 0x2, &u64::to_le_bytes(0x1234567812345678).to_vec()) {
            Some(BusException::StoreAddressMisaligned) => (),
            _ => assert!(false),
        }
        assert!(sram.fetch(8, 0x2).is_err());

        /* Unaligned access */
        match sram.store(8, 0x3, &u64::to_le_bytes(0x1234567812345678).to_vec()) {
            Some(BusException::StoreAddressMisaligned) => (),
            _ => assert!(false),
        }
        ...
        assert!(sram.fetch(8, 0x5).is_err());

        /* Unaligned access */
        match sram.store(8, 0x6, &u64::to_le_bytes(0x1234567812345678).to_vec()) {
            Some(BusException::StoreAddressMisaligned) => (),
            _ => assert!(false),
        }
        assert!(sram.fetch(8, 0x6).is_err());

        /* Unaligned access */
        match sram.store(8, 0x7, &u64::to_le_bytes(0x1234567812345678).to_vec()) {
            Some(BusException::StoreAddressMisaligned) => (),
            _ => assert!(false),
        }
